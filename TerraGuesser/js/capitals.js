...
function printOutput(inputValue, outElement) {
  ...
  const closestMatchResult = closestMatch2(
    inputValue,
    data.map((c) => c.capital)
  );

  const inputLat = data.find((c) => c.capital === closestMatchResult).capitalLat;
  const inputLon = data.find((c) => c.capital === closestMatchResult).capitalLon;

  const dist = Math.round(100 * distanceKm(target.capitalLat, target.capitalLon, inputLat, inputLon)) / 100;
  clone.querySelector(".name").textContent = closestMatchResult + " - " + dist + " km";
  clone.querySelector(".flag").src =
    "https://flagcdn.com/" + data.find((c) => c.capital === closestMatchResult).countryCode + ".svg";
